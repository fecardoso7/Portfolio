...
const ProjectCard = memo(({ project, language, onOpen }) => {
  ...
  // Seleciona idioma sem disparar re-cálculos pesados
  ...
  // Checa se o texto transbordou para mostrar o botão "Mais"
  useLayoutEffect(() => {
  ...
  return (
    <article className="group relative flex flex-col p-6 rounded-[2rem] bg-white dark:bg-[#080808]/40 border border-slate-200/60 dark:border-white/[0.04] backdrop-blur-sm transition-all duration-500 hover:border-blue-500/30 overflow-hidden transform-gpu">
      ...
      {/* Container da imagem com loader e efeito de escala */}
      ...
      <div className="flex-1 relative z-10">
        {/* Descrição com controle de expansão */}
        <div className="relative mb-6">
          <p
            ref={textRef}
            className={`text-slate-600 dark:text-white/50 text-[11px] leading-relaxed transition-all duration-300 ${expanded ? "line-clamp-none" : "line-clamp-2"}`}
          >
            {description}
          </p>
        ...
        <div className="flex gap-3 mt-auto">
          <button
            className="flex-1 flex items-center justify-center gap-2 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-[10px] font-bold tracking-widest uppercase transition-all shadow-lg active:scale-95"
            onClick={() => onOpen(project.liveUrl)}
          >
            <ExternalLink size={14} /> Demo
          </button>
          <button
            className="p-3 bg-white dark:bg-white/[0.03] text-slate-400 hover:text-blue-500 border border-slate-200 dark:border-white/[0.05] rounded-xl transition-all active:scale-95"
