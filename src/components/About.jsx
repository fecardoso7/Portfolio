// About.jsx
// Seção About pensada para:
// - scroll snap previsível
// - layout estável (sem reflow desnecessário)
// - hierarquia visual clara
// - microinterações leves e GPU-friendly
// - código legível sem sacrificar performance
...
const About = () => {
  ...
  /* ============================
     📊 Stats (dependem só do idioma)
     useMemo aqui faz sentido: evita recriação de objetos
     ============================ */
  const stats = useMemo(
    () => [
      {
  ...
  return (
    <section
      id="about"
      // Full-screen com fallback responsivo + scroll snap
    ...
    >
      {/* =========================================================
         🎨 BACKGROUND LAYER
         Camada isolada para evitar impacto no layout principal
         ========================================================= */}
      ...
      {/* =========================================================
         🧭 SECTION HEADER
         Mantém consistência visual com Hero/Skills
         ========================================================= */}
      ...
      <div className="w-full max-w-6xl mx-auto px-6 relative z-10 flex items-center justify-center">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-20 items-center w-full">
          {/* ============================
             🖼️ PROFILE IMAGE
             Isolada para evitar reflow do texto
             ============================ */}
          <div className="lg:col-span-5 relative group flex justify-center lg:justify-start order-2 lg:order-1 transform-gpu">
            ...
            <div
              className="relative w-full max-w-[240px] sm:max-w-[320px] xl:max-w-[400px] 
                            rounded-[2rem] overflow-hidden border border-slate-200/50 
                            dark:border-white/[0.05] bg-slate-50 dark:bg-[#080808] transition-all"
            >
              <img
                src="/profile.webp"
                alt="Profile"
          ...
          <div className="lg:col-span-7 space-y-6 lg:space-y-10 order-1 lg:order-2">
            {/* Descrição principal */}
            <p
              className="text-sm md:text-xl xl:text-2xl text-slate-600 dark:text-slate-400 
                          leading-relaxed font-light tracking-tight text-center lg:text-left"
            >
              {t.about.description}
            </p>
            ...
            {/* ============================
               ...
               ============================ */}
            <div
              className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 pt-6 
                            border-t border-slate-100 dark:border-white/[0.03]"
            >
              {stats.map((stat) => {
                ...
                return (
                  <div
                    ...
                    className="group/stat relative p-4 md:p-5 rounded-2xl 
                               ...
                               transition-all duration-500 hover:bg-blue-600/10 
                               hover:border-blue-500/20 hover:scale-[1.05] 
                               transform-gpu flex flex-col items-center lg:items-start cursor-default"
                  >
                    {/* Ícone */}
                    <div
                      className="text-blue-500/80 mb-2 md:mb-3 transition-all duration-500 
                                    group-hover/stat:text-blue-400 group-hover/stat:scale-110"
                    >
                      <Icon size={18} />
                    </div>
                    ...
                    {/* Label */}
                    <p
                      className="text-[9px] md:text-[10px] uppercase tracking-[0.4em] font-bold 
                                  text-slate-400 dark:text-slate-500 mb-1 md:mb-2 
                                  transition-colors duration-500 group-hover/stat:text-blue-400"
                    >
                      {stat.label}
                    </p>
                    ...
                    {/* Valor */}
                    <p
                      className="text-[12px] md:text-[13px] font-bold text-slate-900 
                                  dark:text-slate-100 leading-tight transition-colors duration-500 
      ...
      <div
        className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full max-w-5xl h-px 
                      ...
                      dark:via-blue-400/10 to-transparent"
      />
    </section>
  );
};
